...
export class MsdbEffects {
    ...
    reduxifyMsdb$: Observable<Actioning> = this.actions$.ofType(EFFECT_INIT_REDUXIFY_MSDB)
        .do(() => {
    ...
    // @Effect()
    // getStations: Observable<Actioning> = this.actions$.ofType(EFFECT_GET_STATIONS)
    //     .debounceTime(500)
    //     .mergeMap(({payload}) => {
